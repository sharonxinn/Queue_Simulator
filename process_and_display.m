function process_and_display(data, label)
    ...
    for i = 1:size(data,1)
        ...
        if assigned_pump == 1
            refuel_str = num2str(refuel);
            begin_str = num2str(begin_time);
            end_str = num2str(end_time);
        else
            refuel_str = '-';
            begin_str = '-';
            end_str = '-';
        ...
        fprintf(['%10d | %-11s | %13.2f | %17.2f | %18d | %13s | %13d | %4d | ', ...
                ...
                data{i,8}, line_no, data{i,9}, refuel_str, begin_str, end_str);
    end

    fprintf('\n\n');
    fprintf('%-17s%-46s%-42s%-40s\n', '', '                  Pump 2:', '                  Pump 3:', '                  Pump 4:');
